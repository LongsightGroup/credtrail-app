...
const getDefaultCourseId = () => {
  ...
  const fromTemplate = templateId.length > 0 ? badgeTemplateCourseMap.get(templateId) : null;
...
const conditionTypeHelpText = {
  ...
  grade_threshold: "Learner score must meet the configured minimum and/or maximum threshold.",
  program_completion:
  ...
  time_window: "Badge can only be earned inside the configured date-time window.",
  prerequisite_badge: "Learner must already hold a specific prerequisite badge.",
  custom_field: "Learner must match an institution-specific field from imported or connected data.",
};
