...
export const encodeRevocationBitstring = async (
  ...
  const dynamicBitsetLength = maxStatusListIndex < 0 ? 1 : Math.floor(maxStatusListIndex / 8) + 1;
  const bitsetLength = Math.max(dynamicBitsetLength, MINIMUM_STATUS_LIST_BYTES);
