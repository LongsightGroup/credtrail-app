...
export const createCredentialVerificationChecks = <ContextType>(
  ...
  const collectKnownContextUrlTerms = (
  ...
  const verifyCredentialJsonLdSafeModeSummary = (
    ...
    collectKnownContextUrlTerms(contextUrls, knownTerms);
  ...
  const hasCredentialSubjectIdentifier = (credentialSubject: JsonObject): boolean => {
    const hasIdentifierObjectValue = (value: unknown): boolean => {
      const identifierEntryAsObject = input.asJsonObject(value);

      if (identifierEntryAsObject === null) {
        return false;
      }

      return (
        input.asNonEmptyString(identifierEntryAsObject.identifier) !== null ||
        input.asNonEmptyString(identifierEntryAsObject.identityHash) !== null
      );
    };
    ...
    if (Array.isArray(identifierValue)) {
      return identifierValue.some((entry) => {
        ...
        return hasIdentifierObjectValue(entry);
      });
    }

    return hasIdentifierObjectValue(identifierValue);
  };
