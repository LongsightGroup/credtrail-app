...
const registerEnterpriseAuthCompatPost = (
  app: Hono<AppEnv>,
  currentPath: string,
  legacyPath: string,
  handler: (c: AppContext) => Promise<Response>,
): void => {
  app.post(currentPath, handler);
  app.post(legacyPath, handler);
};
...
export const registerTenantAccessEnterpriseAdminRoutes = (
  ...
  registerEnterpriseAuthCompatPost(
    app,
    "/tenants/:tenantId/admin/access/authentication/policy",
    "/tenants/:tenantId/admin/access/governance/enterprise-auth/policy",
    async (c) => {
  ...
  registerEnterpriseAuthCompatPost(
    app,
    "/tenants/:tenantId/admin/access/authentication/providers",
    "/tenants/:tenantId/admin/access/governance/enterprise-auth/providers",
    async (c) => {
      const pathParams = parseTenantPathParams(c.req.param());
      const nextPath = accessAuthenticationPageUrl(pathParams.tenantId);
      const roleCheck = await resolveInstitutionAdminAdminRole(c, pathParams.tenantId, nextPath);
  ...
  registerEnterpriseAuthCompatPost(
    app,
    "/tenants/:tenantId/admin/access/authentication/providers/delete",
    "/tenants/:tenantId/admin/access/governance/enterprise-auth/providers/delete",
    async (c) => {
      const pathParams = parseTenantPathParams(c.req.param());
      const nextPath = accessAuthenticationPageUrl(pathParams.tenantId);
      const roleCheck = await resolveInstitutionAdminAdminRole(c, pathParams.tenantId, nextPath);
  ...
  registerEnterpriseAuthCompatPost(
    app,
    "/tenants/:tenantId/admin/access/authentication/break-glass-accounts",
    "/tenants/:tenantId/admin/access/governance/break-glass-accounts",
    async (c) => {
      const pathParams = parseTenantPathParams(c.req.param());
      const nextPath = accessAuthenticationPageUrl(pathParams.tenantId);
      const roleCheck = await resolveInstitutionAdminAdminRole(c, pathParams.tenantId, nextPath);
  ...
  registerEnterpriseAuthCompatPost(
    ...
    "/tenants/:tenantId/admin/access/governance/break-glass-accounts/revoke",
    async (c) => {
      const pathParams = parseTenantPathParams(c.req.param());
      const nextPath = accessAuthenticationPageUrl(pathParams.tenantId);
      const roleCheck = await resolveInstitutionAdminAdminRole(c, pathParams.tenantId, nextPath);
