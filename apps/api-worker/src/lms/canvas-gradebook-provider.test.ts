...
describe("createCanvasGradebookProvider", () => {
  it("maps canvas API responses to normalized records", async () => {
    ...
    const courseSearch = await provider.courseCatalogForUser("instructor-1").listCourses({
      limit: 100,
    });
  ...
  it("searches the complete authorized course set across pages", async () => {
    ...
    const result = await provider.courseCatalogForUser("instructor-1").listCourses({
      searchTerm: "capstone",
      limit: 100,
    });
  ...
  it("filters the authorized Canvas course set by title, code, or ID", async () => {
    ...
    await expect(
      provider.courseCatalogForUser("instructor-1").listCourses({
        searchTerm: " cap-401 ",
        limit: 100,
      }),
    ).resolves.toEqual({
  ...
  it("verifies course access against the instructor's active teacher enrollments", async () => {
    ...
    await expect(
      provider.courseCatalogForUser("instructor-1").verifyCourseAccess({
        courseIds: ["42", "77"],
      }),
    ).resolves.toEqual({
      authorizedCourses: [
