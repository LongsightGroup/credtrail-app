
(() => {
  const heroImages = document.querySelectorAll('.public-badge__hero-image');

  for (const image of heroImages) {
    if (!(image instanceof HTMLImageElement)) {
      continue;
    }

    image.addEventListener('error', () => {
      const fallback = image.dataset.fallbackSrc;

      if (typeof fallback === 'string' && fallback.length > 0 && image.src !== fallback) {
        image.src = fallback;
        delete image.dataset.fallbackSrc;
        return;
      }

      image.hidden = true;
      image.parentElement?.setAttribute('data-fallback', 'true');
    });
  }

  const copyBadgeButton = document.getElementById('copy-badge-url-technical-button');
  const copyBadgeStatus = document.getElementById('copy-badge-url-status');

  if (copyBadgeButton instanceof HTMLButtonElement && copyBadgeStatus instanceof HTMLElement) {
    const value = copyBadgeButton.dataset.copyValue;

    if (typeof value === 'string' && value.length > 0) {
      copyBadgeButton.addEventListener('click', async () => {
        try {
          await navigator.clipboard.writeText(value);
          copyBadgeStatus.textContent = 'Public URL copied';
        } catch {
          copyBadgeStatus.textContent = 'Unable to copy public URL automatically';
        }
      });
    }
  }

  const chapiLink = document.getElementById('chapi-store-link');
  const chapiRow = document.getElementById('chapi-store-row');
  const chapiStatus = document.getElementById('chapi-store-status');

  const supportsBrowserWalletImport = () => {
    if (!globalThis.isSecureContext) {
      return false;
    }

    if (navigator.webdriver === true) {
      return false;
    }

    const credentialsApi = navigator.credentials;
    if (credentialsApi === undefined || typeof credentialsApi.store !== 'function') {
      return false;
    }

    const userAgent = navigator.userAgent;
    const isChromiumFamily = new RegExp('Chrome|Chromium|Edg/').test(userAgent);
    const isIosSafari =
      /iPhone|iPad|iPod/.test(userAgent) && /Safari/.test(userAgent) && !isChromiumFamily;

    return isChromiumFamily && !isIosSafari;
  };

  const revealBrowserWalletImport = async () => {
    if (
      !(chapiLink instanceof HTMLButtonElement) ||
      !(chapiRow instanceof HTMLElement) ||
      !(chapiStatus instanceof HTMLElement)
    ) {
      return;
    }

    if (!supportsBrowserWalletImport()) {
      return;
    }

    const credentialJsonUrl = chapiLink.dataset.credentialJsonUrl;
    if (typeof credentialJsonUrl !== 'string' || credentialJsonUrl.length === 0) {
      return;
    }

    try {
      const response = await fetch(credentialJsonUrl, {
        headers: {
          accept: 'application/vc+ld+json, application/ld+json, application/json',
        },
      });

      if (!response.ok) {
        return;
      }

      const contentType = response.headers.get('content-type') ?? '';
      if (
        contentType.length > 0 &&
        !contentType.includes('json') &&
        !contentType.includes('ld+json')
      ) {
        return;
      }

      await response.json();
      chapiRow.hidden = false;
    } catch {
      return;
    }

    chapiLink.addEventListener('click', async () => {
      const credentialsApi = navigator.credentials;

      if (credentialsApi === undefined || typeof credentialsApi.store !== 'function') {
        chapiStatus.textContent =
          'Browser wallet import is unavailable here. Open in wallet app or download JSON-LD instead.';
        return;
      }

      try {
        const response = await fetch(credentialJsonUrl, {
          headers: {
            accept: 'application/vc+ld+json, application/ld+json, application/json',
          },
        });

        if (!response.ok) {
          chapiStatus.textContent = 'Unable to load credential. Download JSON-LD instead.';
          return;
        }

        const credential = await response.json();

        await credentialsApi.store({
          type: 'OpenBadgeCredential',
          credential,
        });
        chapiStatus.textContent = 'Credential sent to this browser.';
      } catch {
        chapiStatus.textContent =
          'No browser wallet accepted this credential. Open in wallet app or download JSON-LD instead.';
        chapiRow.hidden = true;
      }
    });
  };

  void revealBrowserWalletImport();

  const copyWallButtons = document.querySelectorAll('.badge-wall__icon-button[data-copy-value]');

  for (const button of copyWallButtons) {
    if (!(button instanceof HTMLButtonElement)) {
      continue;
    }

    const copyValue = button.dataset.copyValue;
    const status = button.closest('.badge-wall__item')?.querySelector('.badge-wall__copy-status');

    if (typeof copyValue !== 'string' || copyValue.length === 0 || !(status instanceof HTMLElement)) {
      continue;
    }

    button.addEventListener('click', async () => {
      try {
        await navigator.clipboard.writeText(copyValue);
        status.textContent = 'Copied link';
      } catch {
        status.textContent = 'Unable to copy automatically';
      }
    });
  }
})();
